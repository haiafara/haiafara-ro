...
RSpec.describe PhotoSerializer do
  ...
  before do
    allow_any_instance_of(Photo).to receive(:image_url).with(:thumbnail).and_return(thumbnail_image_url)
    allow_any_instance_of(Photo).to receive(:image_url).with(:large).and_return(large_image_url)
  end
